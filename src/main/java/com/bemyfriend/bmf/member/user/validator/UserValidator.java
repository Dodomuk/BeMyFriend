package com.bemyfriend.bmf.member.user.validator;
...
public class UserValidator implements Validator{
	...
	public UserValidator(UserRepository memberRepository) {
	...
	public void validate(Object target, Errors errors) {
		...
		if(userRepository.selectMemberById(persistInfo.getUserId()) != null){
			errors.rejectValue("userId", "error.userId", "이미 존재하는 아이디 입니다.");
		}
		
		//2. password가 8자리 이상 숫자, 영문자, 특수기호가 각 하나씩 들어있는지
		if(!pattern.matcher(persistInfo.getUserPw()).find()) {
			errors.rejectValue("userPw", "error.userPw", "비밀번호는 숫자,영문자,특수문자 조합의 8글자 이상인 문자열입니다.");
